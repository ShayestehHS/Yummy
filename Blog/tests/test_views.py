...
class PublicBlogViewsTest(TestCase):
    def setUp(self):
        ...
        self.setUP_blog = Blog.objects.create(
            author=self.setUp_user, title='blog test',
    ...
    def test_all_primary_blogs_listing(self):
        ...
        blog = Blog.objects.create(author=self.setUp_user, title='primary',
        ...
        self.assertEqual(blogs_titles, [self.setUP_blog.title, blog.title])
    ...
    def test_blog_detail_page_is_working(self):
        ...
        res = self.client.get(reverse('detail_blog',
                                      kwargs={'blog_id': self.setUP_blog.id, }))
        detail_blog = res.context['blog']

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertTemplateUsed(res, 'blog/post_detail.html')
        self.assertEqual(detail_blog, self.setUP_blog)
    ...
    def test_blog_tag_search_is_working(self):
        ...
        blog = Blog.objects.create(
            author=self.setUp_user, title='blog tag search',
            blog_text='This is a long text', picture=self.picture,
            is_approved=True,
        )
        self.setUP_blog.tags.add('test_tag')
        ...
        self.assertEqual(res_blogs[0].title, self.setUP_blog.title)
    ...
    def test_blog_search_is_working(self):
        ...
        res = self.client.post(reverse('blog_search'), data={
            'searched_key': self.setUP_blog.title, })
        ...
        self.assertIn(self.setUP_blog, list(founded_blogs))
